...
def scrape_mbb_masmovil_data():
    urls = {
        "Prepaid": [
            "https://www.masmovilpanama.com/prepago/planes"
        ],
        "Postpaid": [
            "https://www.masmovilpanama.com/planes-moviles/postpago"
        ]
    }
    ...
    with pd.ExcelWriter('MBB_masmovil_plans_full.xlsx', engine='xlsxwriter') as writer:
        for country, links in urls.items():
            for link in links:
                try:
                    ...
                    if "Postpaid" in country and new_lines == False:

                        # Encontrar el grupo de botones por aria-label
                        grupo_toggle = driver.find_element(By.CLASS_NAME,
                                                           "dynamic-filters__lines")
                    ...
                    plans = soup.find_all("div", class_="card-body")
