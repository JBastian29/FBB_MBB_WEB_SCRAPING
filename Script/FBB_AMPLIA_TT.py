...
def scrape_amplia_data():
    urls = {
        "bundles": [
            "https://amplia.co.tt/smart-bundles"
        ],
        "internet": [
    ...
    with pd.ExcelWriter('FBB_amplia_plans_full.xlsx', engine='xlsxwriter') as writer:
        for country, links in urls.items():
            for link in links:
                try:
                    ...
                    plans = soup.find_all("div", class_="mmg-package-body")
