...
def scrape_telecable_data():
    urls = {
        "ALL internet": [
            "https://telecablecr.com/residencial/combos/"
        ]
    }
    ...
    #tripleplay_selected = False

    # URL de Digicel Jamaica (asegúrate de colocar la correcta)

    with pd.ExcelWriter('FBB_telecable_plans_full.xlsx', engine='xlsxwriter') as writer:
        for country, links in urls.items():
            for link in links:
                try:
                    ...
                    #if tripleplay_selected == False:
                    #    elemento = driver.find_element(By.ID, "jet-tabs-control-1584")
                       # Hacer scroll hasta el elemento (opcional si está fuera de la vista)
                    #    driver.execute_script("arguments[0].scrollIntoView();", elemento)
                        # Hacer clic en el elemento
                    #    driver.execute_script("arguments[0].click();", elemento)
                    #    tripleplay_selected = True
